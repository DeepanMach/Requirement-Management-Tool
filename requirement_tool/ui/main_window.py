...
_HEADING_LABEL_RE = re.compile(r"^heading\s*\d+\s*-\s*([0-9.]+)", re.IGNORECASE)

def _heading_level_from_type(obj_type: str) -> int | None:
    m = re.search(r"heading\s*(\d+)", str(obj_type or ""), re.IGNORECASE)
    return int(m.group(1)) if m else None

def _section_label_from_type(obj_type: str) -> str:
...
class ExcelTab(QWidget):
    ...
    def load_files(self, files: list[str]) -> None:
        """
        Use backend engine to load all Excel data (with image extraction, markdown->table
        conversion, auto captions, renumbering, and summary popup).
        """
        try:
            self.engine.load_from_paths(files)        # does extraction + captions + renumber + popup
            self.df = self.engine.get_dataframe()
    ...
    def build_preview_html(self) -> str:
        ...
        embedded images, numbered captions). No UI duplication.
        """
        if self.df is None or self.df.empty:
            return "<div style='font-family:Segoe UI;'>[No data loaded]</div>"
        try:
            # Ensure engine renders the exact dataframe currently in the tab
            self.engine.load_from_dataframe(self.df)
            return self.engine.generate_preview_html()
        except Exception:
            return "<div style='font-family:Segoe UI;'>[Preview unavailable]</div>"
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, data_manager: Optional[RequirementDataManager] = None):
        ...
        self.excel_engine = ExcelPreviewWidget()
        self.excel_engine.dataChanged.connect(self._on_engine_data_changed)
...
# (dedup)         self.search_results.itemClicked.connect(self.navigate_to_search_result)
    ...
    def _on_engine_data_changed(self, df):
        # Refresh whichever tab is showing the table
        tab = getattr(self, "table_tabs", None)
        if tab:
            w = tab.currentWidget()
            if w and hasattr(w, "df"):
                w.df = df
                if hasattr(w, "_populate_table"):
                    w._populate_table()
    ...
    def _get_dataframe_for_source(
        ...
        if key in self._tab_indices:
            if raw:
                # Ignore per-tab filters/sorts and return the whole group
                if key == "Excel":
                    ...
                    if "SourceFile" in df.columns:
                        mask = df["SourceFile"].astype(str).str.strip().eq(key)
                        return df[mask].copy()
            indices = self._tab_indices.get(key, [])
            return df.loc[indices].reset_index(drop=True).copy() if indices else pd.DataFrame()
            if indices:
    ...
    def populate_navigation(self) -> None:
        """
        Build the left-side navigation tree based on heading levels.
        Handles missing 'Section Number' column, and Object Type formats like 'Heading 1 - 10.1'.
        """
        self.nav_tree.clear()
        ...
        for global_index, row in subset.iterrows():
            ...
            lvl = _heading_level_from_type(obj_type)
            if not lvl:
                continue  # Skip non-heading rows

            # --- Safe section + text extraction ---
            section = str(row.get(section_col, "") or "").strip() or _section_label_from_type(obj_type)
            text = str(row.get("Object Text", "") or "").strip()
            if not text and not section:
                continue

            label = f"{section} {text}".strip() if section else text
            payload = {
            ...
            if lvl == 1:
                ...
                if current_h1 is None:
                    current_h1 = QTreeWidgetItem(["(Untitled Section)"])
                    self.nav_tree.addTopLevelItem(current_h1)
                ...
                if current_h2 is None:
                    if current_h1 is None:
                        current_h1 = QTreeWidgetItem(["(Untitled Section)"])
                        self.nav_tree.addTopLevelItem(current_h1)
                    current_h2 = QTreeWidgetItem(["(Subsection)"])
                    current_h1.addChild(current_h2)
    ...
    def compose_preview_html(self, df: Optional[pd.DataFrame] = None) -> str:
        ...
        for idx, row in local_df.iterrows():
            ...
            if not section:
                section = _section_label_from_type(obj_type)
            if not section:
            ...
            # --- Headings ---
            if lower_type.startswith("heading"):
            ...
            if req_id:
                # Build safe HTML anchor for this requirement ID
                rid = str(req_id).strip()
                rid_anchor = re.sub(r'[^A-Za-z0-9_.-]+', '-', rid)  # normalize ID for HTML anchor
                anchor_id = f"rid-{rid_anchor}"

                # --- Make Up/Down Trace text clickable ---
                def make_trace_links(trace_text: str) -> str:
                    tokens = re.split(r"[,;\\s]+", trace_text.strip())
                    linked = []
                    for t in tokens:
                        t = t.strip()
                        if not t:
                            continue
                        safe_id = re.sub(r'[^A-Za-z0-9_.-]+', '-', t)
                        linked.append(f'<a href="#rid-{safe_id}">{html.escape(t)}</a>')
                    return ", ".join(linked)

                up_trace = str(row.get("Up Trace", "") or "").strip()
                down_trace = str(row.get("Down Trace", "") or "").strip()
                up_html = make_trace_links(up_trace) if up_trace else ""
                down_html = make_trace_links(down_trace) if down_trace else ""

                # --- Build requirement block ---
                requirement_text = html.escape(text) if text else "&nbsp;"
                body_parts.append(
                    f'<div id="{anchor_id}" class="requirement-block">'
                    f'<h4><a href="#{anchor_id}">{html.escape(rid)}</a></h4>'
                    f'<p class="requirement-text">{requirement_text}</p>'
                )
                if up_html or down_html:
                    body_parts.append("<p>")
                    if up_html:
                        body_parts.append(f"<b>Up Trace:</b> {up_html}<br/>")
                    if down_html:
                        body_parts.append(f"<b>Down Trace:</b> {down_html}")
                    body_parts.append("</p>")
        ...
        if toc_entries:
            ...
            for entry in toc_entries:
                toc_html += (
                    ...
                    f'<a href="#{entry["id"]}">{html.escape(entry["title"])}</a></div>'
                )
            toc_html += "</div>"
    ...
    def run_convert_preview(self):
        """
        When user clicks 'Run (Convert to Word)', build the right-hand HTML preview
    ...
    def add_trace_column(self):
        ...
        col = self.data_manager.ensure_trace_column(direction)
        self._refresh_table_view()
        if hasattr(self, "log_console"):
            self.log_console(f"Trace column '{col}' added to table.")
    ...
    def add_excel_tab_from_files(self, files: list[str]) -> None:
        ...
        if not files:
            return
        tab = ExcelTab(self, engine=self.excel_engine)
