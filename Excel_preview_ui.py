    ...
    def deleteLater(self) -> None:  # pragma: no cover
        try:
            ...
            try:
                ...
                pass
# ======================================================
# Standalone demo UI when run directly (non-invasive)
# ======================================================
if __name__ == "__main__":
    ...
    from PyQt6.QtWidgets import (
        ...
        QTableView, QTextBrowser, QFileDialog, QMessageBox, QToolBar, QLabel, QStatusBar
    )

    class _DfView(QTableView):
        """Lightweight table view helper to show a pandas DataFrame."""
        def set_dataframe(self, df: pd.DataFrame):
            ...
            for c, name in enumerate(df.columns):
                model.setHeaderData(c, Qt.Orientation.Horizontal, str(name))
            # cells
    ...
    class DemoWindow(QMainWindow):
        """
        Minimal demo shell around ExcelPreviewWidget for validation.
        Left: table view of dataframe
        Right: HTML preview (images/tables/headings with captions)
        Top: toolbar (Load / Preview / Undo / Insert Image / Insert Table / Feature Check)
        """
        def __init__(self):
            super().__init__()
            self.setWindowTitle("ExcelPreviewWidget — Demo Harness")
            self.resize(1200, 700)

            # backend engine (your widget logic only; no UI inside)
            self.engine = ExcelPreviewWidget()
            self.engine.dataChanged.connect(self._on_engine_changed)
            ...
            act_load = QAction("Load Excel(s)", self)
            act_load.triggered.connect(self._act_load)

            act_preview = QAction("Preview", self)
            act_preview.setShortcut(QKeySequence("Ctrl+P"))
            act_preview.triggered.connect(self._act_preview)

            act_undo = QAction("Undo", self)
            act_undo.setShortcut(QKeySequence("Ctrl+Z"))
            act_undo.triggered.connect(self._act_undo)

            act_ins_img = QAction("Insert Image…", self)
            act_ins_img.triggered.connect(self._act_insert_image)

            act_ins_tbl = QAction("Insert Table…", self)
            act_ins_tbl.triggered.connect(self._act_insert_table)

            act_check = QAction("Feature Check", self)
            act_check.triggered.connect(self._act_feature_check)

            tb.addAction(act_load)
            tb.addAction(act_preview)
            tb.addAction(act_undo)
            tb.addSeparator()
            tb.addAction(act_ins_img)
            tb.addAction(act_ins_tbl)
            tb.addSeparator()
            tb.addAction(act_check)
        ...
        def _act_load(self):
            ...
            if not paths:
                return
            try:
                # This triggers: image extraction, MD→table conversion, auto captions, renumbering, summary popup
                self.engine.load_from_paths(paths)
                self._refresh_views()
                self.statusBar().showMessage(
                    f"Loaded {len(paths)} file(s)", 3000
                )
            except Exception as e:
                QMessageBox.critical(self, "Load Excel Error", str(e))

        def _act_preview(self):
            html = self.engine.generate_preview_html()
        ...
        def _act_insert_image(self):
            ...
            if not img:
                return
            try:
                # load current df first (so insertion appends to current data)
                self.engine.load_from_dataframe(self.engine.get_dataframe())
                self.engine.insert_image_row(img, name=Path(img).stem)
                self._refresh_views()
                self.statusBar().showMessage("Inserted image (with caption)", 3000)
            except Exception as e:
                QMessageBox.critical(self, "Insert Image Error", str(e))

        def _act_insert_table(self):
            csv, _ = QFileDialog.getOpenFileName(
                self, "Select CSV (as Table)", "", "CSV Files (*.csv);;All Files (*.*)"
            )
            if not csv:
                return
            try:
                try:
                    df_tbl = pd.read_csv(csv)
                except Exception:
                    df_tbl = pd.DataFrame([["", ""], ["", ""]])
                self.engine.load_from_dataframe(self.engine.get_dataframe())
                self.engine.insert_table_row(df_tbl, name=Path(csv).stem)
                self._refresh_views()
                self.statusBar().showMessage("Inserted table (with caption)", 3000)
            except Exception as e:
                QMessageBox.critical(self, "Insert Table Error", str(e))

        def _act_feature_check(self):
            checks = {
                "Embedded image extraction": hasattr(self.engine, "_extract_embedded_images_per_sheet"),
                "Auto caption insertion"   : hasattr(self.engine, "insert_image_row") and hasattr(self.engine, "insert_table_row"),
                "Undo (Ctrl+Z)"            : hasattr(self.engine, "undo"),
                "Load summary popup"       : hasattr(self.engine, "load_from_paths"),
                "Preview HTML generation"  : hasattr(self.engine, "generate_preview_html"),
                "Public API OK"            : all(hasattr(self.engine, fn) for fn in [
                                                "load_from_paths", "load_from_path",
                                                "load_from_dataframe", "get_dataframe"
                                              ]),
            }
            # Count images/tables currently present
                ...
                for _, row in df.iterrows():
                    for val in row.values:
                        if is_image_marker(val):
                            ...
                            tbl_count += 1
            msg = ["<b>ExcelPreviewWidget Feature Check</b><br><br>"]
            for key, ok in checks.items():
                msg.append(f"{key}: {'✅' if ok else '❌'}")
            msg.append("<br>")
            msg.append(f"Images detected (markers): <b>{img_count}</b>")
            msg.append(f"Tables detected (markers): <b>{tbl_count}</b>")
            QMessageBox.information(self, "Feature Check", "<br>".join(msg))

        # -------- engine → UI
        def _on_engine_changed(self, df: pd.DataFrame):
            self._refresh_views()

        def _refresh_views(self):
            df = self.engine.get_dataframe()
            self.table.set_dataframe(df if df is not None else pd.DataFrame())
            self.preview.setHtml(self.engine.generate_preview_html())
